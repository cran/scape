plotIndex <- function(model, what="s", series=NULL, axes=TRUE,
                      same.limits=FALSE, between=list(x=axes,y=axes), ylim=NULL,
                      q=1, bar=1, log=FALSE, base=10, main="", xlab="", ylab="",
                      ...
                      col.grid="white", pch=16, cex.points=1.2,
                      ...
                      col.lines="dimgray", lty.bar=1, plot=TRUE, ...)
{
                          ...
                          col.lines, ...)  # overlay obs&fit in series panels
  {
    ...
    panel.abline(v=pretty(x,tick.number), h=pretty(y.range,tick.number),
                 lty=lty.grid, col=col.grid)
    panel.xyplot(x, y, type="n", ...)
    panel.xyplot(x, yfit[subscripts], type="l", lty=lty.lines, lwd=lwd.lines,
                 col=col.lines[subscripts], ...)
    if(lty.bar == 0)
      panel.xyplot(x, yobs[subscripts], col=col.points, ...)
    else
      panel.xYplot(x, yobs[subscripts], subscripts=subscripts,
  ...
  graph <- xyplot(Fit~Year|factor(Series), data=x, panel=panel.index,
                  yobs=Cbind(x$Obs,x$Hi,x$Lo), yfit=x$Fit, as.table=TRUE,
                  between=between, main=mymain, xlab=myxlab, ylab=myylab,
                  scales=myscales, strip=strip, par.strip.text=mytext, pch=pch,
                  cex=cex.points, col.points=col.points[factor(x$Series)],
  ...
  {
    extremes <- as.data.frame(lapply(split(x[,c("Fit","Hi","Lo")],x$Series),
                                     range, na.rm=TRUE))
    if(!log)
      graph$y.limits <- lapply(extremes, function(x) c(0,1.04*x[2]))
    else
      graph$y.limits <- lapply(extremes, function(x)
                                         range(x)+c(-0.04,0.04)*diff(range(x)))
  }
  if(same.limits)
