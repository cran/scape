         ...
         col.points="black", lty.lines=1, lwd.lines=2, col.lines=c("red","blue"), plot.it=TRUE, ...)
{
  ...
  if(what == "c")
  {
    ...
    {
      cat("Commercial C@L data (", substitute(model), "$CLc) not found. Assuming what=\"s\" was intended.\n", sep="")
      what <- "s"
    }
    else
      x <- model$CLc
  ...
  if(what == "s")  # value of 'what' may have changed
  {
    if(!any(names(model)=="CLs"))
      stop("Survey C@L data (", substitute(model), "$CLs) not found, try plotCL(x, what=\"c\") to plot commercial C@L")
    x <- model$CLs
  }
  ...
  if(length(series) > 1)
  {
    ...
    cat("More than one C@L series found. Assuming series=\"", series[1], "\" was intended.\n", sep="")
  }
  ...
  if(nsexes==1 && fit)
  {
    graph <- xyplot(P~Length|factor(Year), groups=ObsFit, data=x, panel=panel.fit, type=c("l","p"), as.table=TRUE,
                    main=mymain, xlab=myxlab, ylab=myylab, par.strip.text=mystrip, scales=myscales,
  ...
  {
    x <- x[x$ObsFit=="Obs",]
    graph <- xyplot(P~Length|factor(Year), groups=Sex, data=x, panel=panel.obs, type="l", as.table=TRUE,
                    main=mymain, xlab=myxlab, ylab=myylab, par.strip.text=mystrip, scales=myscales,
                    lty=lty.lines, lwd=lwd.lines, col=col.lines, ...)
  }
  if(nsexes==2 && fit)
  {
    graph <- xyplot(P~Length|factor(Year)*Sex, groups=ObsFit, data=x, panel=panel.fit, type=c("l","p"), as.table=TRUE,
                    main=mymain, xlab=myxlab, ylab=myylab, par.strip.text=mystrip, scales=myscales,
                    pch=pch, cex=cex.points, col.points=col.points[x$Sex], lty=lty.lines, lwd=lwd.lines,
                    col.lines=col.lines[x$Sex], ...)
  }
