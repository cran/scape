plotCL <- function(model, what="c", fit=TRUE, swap=FALSE, series=NULL, sex=NULL,
                   years=NULL, lengths=NULL, axes=TRUE, same.limits=TRUE,
                   ...
                   plot=TRUE, ...)
{
  ...
  if(nsexes==1 && !fit)
  {
    ...
    myformula <- if(!swap)
                   Year~Length|switch(what,c="Commercial C@L","Survey C@L")
                 else
                   Length~Year|switch(what,c="Commercial C@L","Survey C@L")
  ...
  if(nsexes==2 && fit)
  {
    myformula <- if(!swap) P~Length|factor(Year)*factor(Sex)
                 else P~Year|factor(Length)*factor(Sex)
    graph <- xyplot(myformula, data=x, groups=x$ObsFit, panel=panel.fit,
                    type=c("l","p"), as.table=TRUE, main=mymain, xlab=myxlab,
                    ylab=myylab, scales=myscales, strip=strip,
                    par.strip.text=mytext, pch=pch, cex=cex.points,
