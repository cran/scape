plotCA <- function(model, what="c", fit=TRUE, swap=FALSE, series=NULL, sex=NULL,
                   years=NULL, ages=NULL, axes=TRUE, same.limits=TRUE,
                   log=FALSE, base=10, eps.log=1e-5, main="", xlab="", ylab="",
                   cex.main=1.2, cex.lab=1, cex.axis=0.8, cex.strip=0.8,
                   col.strip="gray95", strip=strip.custom(bg=col.strip),
                   las=!fit, tck=c(1,fit)/2, tick.number=5, lty.grid=3,
                   col.grid="gray", pch=16, cex.points=0.5, col.points="black",
                   lty.lines=1, lwd.lines=2, col.lines=c("red","blue"),
                   plot=TRUE, ...)
{
  ## 1  Define functions
  ...
  ## Overlay male and female lines in year panels
  panel.obs <- function(x, y, ...)
  {
    panel.abline(v=pretty(x,tick.number), lty=lty.grid, col=col.grid)
    panel.superpose(x, y, ...)
  }
  ## Overlay obs and fit in sex:year panels
  panel.fit <- function(x, y, subscripts, col.points, col.lines, ...)
  {
    panel.abline(v=pretty(x,tick.number), lty=lty.grid, col=col.grid)
    panel.superpose.2(x, y, subscripts=subscripts,
                      col.symbol=col.points[subscripts],
                      col.line=col.lines[subscripts], ...)
  }

  ## 2  Parse args
  what <- match.arg(what, c("c","s"))
  x <- if(inherits(model,"scape") && what=="c") model$CAc
       else if(inherits(model,"scape") && what=="s") model$CAs
  ...
  {
    x <- x[x$ObsFit=="Obs",]
    ## pch=NA doesn't work if pch is a character
    col.points <- ifelse(x$P==0|is.na(x$P), "transparent", col.points)
    ## cex=0 is illegal on PDF device
    mycex <- cex.points*sqrt(x$P/mean(x$P,na.rm=TRUE)) + 1/1000
    myformula <- if(!swap) Year~Age|switch(what,c="Commercial C@A","Survey C@A")
                 else Age~Year|switch(what,c="Commercial C@A","Survey C@A")
    graph <- xyplot(myformula, data=x, panel=panel.bubble,
                    main=mymain, xlab=myxlab, ylab=myylab, scales=myscales,
                    strip=strip, par.strip.text=mytext, pch=pch, cex=mycex,
                    col=col.points, ...)
  ...
  if(nsexes==1 && fit)
  {
    myformula <- if(!swap) P~Age|factor(Year) else P~Year|factor(Age)
    graph <- xyplot(myformula, data=x, groups=x$ObsFit, panel=panel.fit,
                    type=c("l","p"), as.table=TRUE, main=mymain, xlab=myxlab,
                    ylab=myylab, scales=myscales, strip=strip,
                    par.strip.text=mytext, pch=pch, cex=cex.points,
                    col.points=col.points[factor(x$Sex)], lty=lty.lines,
  ...
  if(nsexes==2 && !fit)
  {
    x <- x[x$ObsFit=="Obs",]
    myformula <- if(!swap) P~Age|factor(Year) else P~Year|factor(Age)
    graph <- xyplot(myformula, data=x, groups=x$Sex, panel=panel.obs, type="l",
  ...
  if(nsexes==2 && fit)
  {
    myformula <- if(!swap) P~Age|factor(Year)*factor(Sex)
                 else P~Year|factor(Age)*factor(Sex)
    graph <- xyplot(myformula, data=x, groups=x$ObsFit, panel=panel.fit,
                    type=c("l","p"), as.table=TRUE, main=mymain, xlab=myxlab,
                    ylab=myylab, scales=myscales, strip=strip,
                    par.strip.text=mytext, pch=pch, cex=cex.points,
